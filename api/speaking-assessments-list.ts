...
export default async function handler(
  ...
  try {
    ...
    // speaking_sessions.user_id is UUID; app_users.id must be UUID (not integer)
    const appUserId = String(appUser.id);
    const uuidLike = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(appUserId);
    if (!uuidLike) {
