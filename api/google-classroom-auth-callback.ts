/**
 * GET /api/google-classroom-auth/callback?code=...&state=...
 * Google redirects here after user authorizes. Exchange code for tokens, store them, redirect to app.
 */
...
export default async function handler(
  ...
  const dashboardUrl = `${baseUrl.replace(/\/$/, '')}/dashboard?google_classroom=connected`;
  const errorUrl = `${baseUrl.replace(/\/$/, '')}/dashboard?google_classroom=error`;
  ...
  try {
    ...
    redirect(res, dashboardUrl, [cookie]);
  } catch (err) {
    console.error('[google-classroom-auth/callback]', err);
    redirect(res, errorUrl);
  }
}
